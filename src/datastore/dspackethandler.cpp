...
/*void DSPacketHandler::prepend_gtp_hdr(uint8_t protocol, uint8_t msg_type, uint16_t msg_len, uint32_t teid) {
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);
...
void DSPacketHandler::prepend_s1ap_hdr(uint8_t msg_type, uint16_t msg_len, uint32_t enodeb_s1ap_ue_id, uint32_t mme_s1ap_ue_id) {
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);	
...
void DSPacketHandler::prepend_diameter_hdr(uint8_t msg_type, uint16_t msg_len) {
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);	
...
/*void DSPacketHandler::extract_gtp_hdr() {
	int hdr_len = GTP_HDR_LEN;
...
void DSPacketHandler::extract_s1ap_hdr() {
	int hdr_len = S1AP_HDR_LEN;
...
void DSPacketHandler::extract_diameter_hdr() {
	int hdr_len = DIAMETER_HDR_LEN;

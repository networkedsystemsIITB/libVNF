				...
				switch(act_type)
				{
					case 1:
						...
						{//MME attach 3
							...
							s11_id[s11_cteid_sgw] = imsi;// sgw.cpp l:308
							s1_id[s1_uteid_ul] = imsi;//update_itfid();
						...
						{//attach 4
							...
							if (imsi == 0) {
								...
								//cout<<"Attach 4 "<<pkt.gtp_hdr.teid<<endl;
								pkt.extract_item(eps_bearer_id);
						...
						{//detach
							//cout<<"In detach "<<pkt.gtp_hdr.teid<<endl;
							if (s11_id.find(pkt.gtp_hdr.teid) != s11_id.end()) {
								imsi = s11_id[pkt.gtp_hdr.teid];
							}
							else
						...
						if(gettid(imsi) != pkt.gtp_hdr.teid)
						{
							...
							pkt.prepend_len();
							//cout<<"imsi "<<imsi<<" TEID "<<s5_cteid_ul<<endl;
					...
					case 3:
						...
						TRACE(cout<<"A3 out "<<imsi << fddata.tid<< " rcv "<<s5_cteid_ul<<endl;)
						
						//restore state sgw.cpp:l98
						...
						//TRACE(cout<<"Info Attach 3: Data send to MME:"<<endl;)
						fdmap.erase(cur_fd);
					...
					case 4:
						...
						TRACE(cout<<"Detacj rcv "<<pkt.gtp_hdr.teid <<" s11"<<s11_cteid_mme<<endl;)
						//s11_cteid_mme = fddata.tid;
						pkt.clear_pkt();
						pkt.append_item(res);
						...
						if(returnval == -1)
						{
							cout<<"Error: mme fd after detach add mtcp error"<<endl;
							exit(-1);
						}		
						...
						returnval = mtcp_epoll_ctl(mctx, epollfd, MTCP_EPOLL_CTL_DEL, mme_fd, &epevent);
						if(returnval == -1)
						{
							cout<<"Error: mme fd delete mtcp error"<<endl;
							exit(-1);
						}
						mtcp_close(mctx, mme_fd);
						//temp
						//closed connection with MME????
						//closed connection with MME????
						//Note, if read in case 1 ret 0, connection closed by mme...
						//closed connection with MME????
						//closed connection with MME????
						//closed connection with MME????
...
void handle_pgw_connect(int cur_fd,Packet pkt,struct mdata fddata, string pgw_s5_ip_addr,int pgw_s5_port)
{
	...
	{
		cout<<"Unexpected happened: PGW connect immedietly, handle this now\n";
		exit(-1);
	}
}
...
void send_request_pgw(int cur_fd, struct mdata fddata)
{
	...
	fdmap.erase(cur_fd);
	//cout<<fddata.idfr<<" check these "<<fddata.initial_fd<<endl;
	fddata.act = 3;
	fddata.buflen = 0;
	memset(fddata.buf, '\0', 500);

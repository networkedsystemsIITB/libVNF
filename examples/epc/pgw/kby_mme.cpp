				...
				switch(act_type)
				{
					...
					case 1:
						...
						if(pkt.s1ap_hdr.msg_type == 4)
						{
							...
							TRACE(cout<<" A4 "<<guti<<" s11_cteid_sgw "<<s11_cteid_sgw<<endl;)
							pkt.clear_pkt();
							pkt.append_item(eps_bearer_id);
						...
						if(pkt.s1ap_hdr.msg_type == 5)
						{
							...
							pkt.prepend_len();
							TRACE(cout<<"DETACH PACKT "<<guti<<" "<<s11_cteid_sgw<<endl;)
							fddata.msui = mme_s1ap_ue_id;
					...
					case 6:
						...
						//temp
						fdmap.erase(ran_fd);						
						returnval = mtcp_epoll_ctl(mctx, epollfd, MTCP_EPOLL_CTL_DEL, ran_fd, &epevent);
						if(returnval == -1)
						{
...
void send_ran_attach_two(int cur_fd, Packet pkt)
{
	...
	//cout<<"Sent Attach 2 confirmation"<<endl;
	//mtcp_close(mctx, cur_fd);
	fddata.act = 1;
	fddata.buflen = 0;
	memset(fddata.buf, '\0', 500);
